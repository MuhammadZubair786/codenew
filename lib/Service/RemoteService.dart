...
// save user token to memory
  ...
  // network call for fetch instructor
  static InstructorFun() async {
    Uri topCatUrl = Uri.parse(baseUrl + '/get-instructor');
  ...
  // network call for fetch instructor course
  static fetchInstructorCoursefunc() async {
    Uri topCatUrl = Uri.parse(baseUrl + '/getInstructorCourses');
    var response = await http.get(topCatUrl);
    // print("hello");
    if (response.statusCode == 200) {
       Map<String, dynamic> map = await json.decode(response.body);
...
// network call for  Slider Pics 
  static SliderPic() async {
    Uri topCatUrl = Uri.parse(baseUrl + '/slider');
    var response = await http.get(topCatUrl);
    if (response.statusCode == 200) {
      Map<String, dynamic> map = await json.decode(response.body);
      List<dynamic> data = map["data"];
...
//add to card
  static Future<CourseMain> addToCard(String token, String courseID) async {
    ...
    var response = await http.get(cartList);
    if (response.statusCode == 200) {
      var jsonString = jsonDecode(response.body);
      var courseData = jsonEncode(jsonString['data']);
      return CourseMain.fromJson(json.decode(courseData));
  ...
  static Future<List<CourseMain>> fetchAllClass() async {
    ...
    var response = await http.get(allCourseUrl);
    if (response.statusCode == 200) {
      var jsonString = jsonDecode(response.body);
      var courseData = jsonEncode(jsonString['data']);
      return List<CourseMain>.from(
