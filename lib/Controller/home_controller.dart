...
class HomeController extends GetxController {
  ...
  fetchInstructorCourse()async{
    try{
      isLoading(true);
      var Instructor= await RemoteServices.fetchInstructorCoursefunc();
    }
    finally{}
  }
  ...
  fetchInstructor()async{
    try{
      isLoading(true);
      var Instructor= await RemoteServices.InstructorFun();
      // print(Instructor[0]);
      if(Instructor!=null){
  ...
  fetchGetCourse()async {
    try {
      isLoading(true);
      var getCourse1 =  await RemoteServices.Getcourse();
      if(getCourse1 !=null){
         GetCourse1.value = getCourse1;
         return GetCourse1;
      }
 } finally {
      isLoading(false);
  ...
  fetchSliderPic()async {
    try {
      isLoading(true);
      var sliderimg =  await RemoteServices.SliderPic();
      if(sliderimg !=null){
         SliderImg.value = sliderimg;
         return SliderImg;
      }
 } finally {
      isLoading(false);
  ...
  void filterCourse(category, level, language, price) async {
    try {
      ...
      if (products != null) {
        courseFiltered(true);
        allCourseText.value =
            products.length.toString() + " " + "کۆرس دۆزرایەوە";
        allCourse.value = products;
      }
    } finally {
      isLoading(false);
